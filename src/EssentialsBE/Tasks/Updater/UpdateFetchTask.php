...
class UpdateFetchTask extends AsyncTask{
    ...
    public function onRun(){
        switch($this->build){
            ...
            default:
                ...
                break;
            case "beta":
                $url = "https://api.github.com/repos/LegendOfMCPE/EssentialsBE/releases"; // Github repository for 'Beta' releases
                break;
            case "development":
                $url = "https://api.github.com/repos/LegendOfMCPE/EssentialsBE/contents/plugin.yml"; // Github repository for 'Development' versions
                break;
        }
        ...
        switch(strtolower($this->build)){
            case "stable":
                ...
                $r["downloadURL"] = "http://forums.pocketmine.net/plugins/EssentialsBE.886/download?version=" . $i["current_version_id"];
                break;
            case "beta":
            ...
            case "development":
                ...
                $r["downloadURL"] = "https://github.com/LegendOfMCPE/EssentialsBE/raw/travis-build/EssentialsBE.phar";
                break;
        }
        $this->setResult($r);
    }
    ...
    public function onCompletion(Server $server){
        /** @var Loader $ess */
        $ess = $server->getPluginManager()->getPlugin("EssentialsBE");
        ...
        if($currentVersion < $v or $this->build === "development"){
            $continue = true;
            $message = TextFormat::AQUA . "[EssentialsBE]" . TextFormat::GREEN .
                ($this->build === "development" ?
                    "Fetching latest EssentialsBE development build..." :
                    " A new " . TextFormat::YELLOW . $this->build . TextFormat::GREEN . " version of EssentialsBE found!"
                ) .
                " Version: " . TextFormat::YELLOW . $v . TextFormat::GREEN .
                ($this->install !== true ? "" : ", " . TextFormat::LIGHT_PURPLE . "Installing...");
        }else{
            $continue = false;
            $message = TextFormat::AQUA . "[EssentialsBE]" . TextFormat::YELLOW . " No new version found, you're using the latest version of EssentialsBE";
        }
